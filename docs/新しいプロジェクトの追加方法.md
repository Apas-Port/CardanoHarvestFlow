...
| `mintPriceLovelace` | ブロックチェーン初期化時 | Oracle の `lovelacePrice` に登録（例: 1969750 = 1.96975 ADA） |
...
`pnpm run scripts:check-oracle <project-id>`
...
| `pnpm run scripts:check-oracle <project-id>` | オラクルに記録されている価格や状態を確認します。 |
